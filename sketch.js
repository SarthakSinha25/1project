...
function setup(){
    ...
    levelComplete = createSprite(displayWidth/2,-170,100,100);
    ...
    if (gameState === 2){
       background(bg_img3)
        ...
        if (player.isTouching(openDoor)){
            console.log("working")
            if (levelComplete.y === 200){
                levelComplete.velocityY = 20
                console.log(levelComplete.y)
            }else{
                levelComplete.velocityY = 0
            }
        }


        if(keyIsDown(UP_ARROW) && player.y >= 616) {
            player.velocityY = -15;
        }
